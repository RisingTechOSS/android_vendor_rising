...
func init() {
	android.RegisterModuleType("rising_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = risingExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = risingExpandVariables(ctx, dep_file)
	...
	cmd := risingExpandVariables(ctx, String(g.properties.Cmd))
